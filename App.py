...
if st.session_state["in_transaction"]:
    ...
    if clicked_commit:
        ...
        # Commit the local transaction first
        conn.commit()
        
        # Add a small delay based on session ID to serialize replication attempts
        import time, hashlib
        session_hash = int(hashlib.md5(st.session_state["session_id"].encode()).hexdigest()[:8], 16)
        delay = (session_hash % 1000) / 1000.0  # 0-1 second delay
        time.sleep(delay)
